...
We use a specific seed (22) to ensure our data runs are reproducible. We perform exploratory data analysis (EDA), caret model selection, and feature selection in an appendix. These steps are as follows:

- Explore the (unaggregated) training time series data to get a feel for what movements were performed for each sequence, as captured by the classe variable.
- After selecting only the rows with aggregated time bin data from the training data, and removing fields that contain no data all all zero and/or all NA values, we explore this subset of the data.
...
- For the aggregated time bin training data, and using all predictors found in the training data, we apply ten models using the caret package to asses the within sample accuracy of predicting the weight-lifting activities using classe as our response variable. We resample from within the training data to obtain confusion matrix results for each caret model. From this work, we determine that four models (Decision Tree, Random Forest, Bagged Trees and SVMpoly-short for Support Vector Machines with polynomial kernel) yield the highest accuracy and kappa measures.
...
## Predicting Classe for the Test Data
...
We attempt using the following models:

- "svmPoly": Least Squares Support Vector Machines with Polynomial Kernel 
...
### run to estimate accuracy with reduced predictor list
...
## Running the training model on the test data for Bagged Trees (minus high corrrelation fields) 
